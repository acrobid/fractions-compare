...
import type { GridCell, GridState } from "./useLongDivision";
...
export function useGridDivision() {
  ...
  const generateLongDivisionSteps = (
    ...
    // Keep track of the row for each working portion
    let currentWorkingPortionRow = currentRow;
    let startingNewWorkingPortion = true;
    ...
    while (
      ...
      while (
        ...
        if (workingPortion === "") {
          ...
          // If this is not the first calculation, increment row for new working portion
          if (!startingNewWorkingPortion) {
            currentRow += 1; // Move to a new row for the start of a new working portion
          }
          currentWorkingPortionRow = currentRow; // Set the row for this new working portion
          startingNewWorkingPortion = false;
        } else {
          // Adding to existing working portion - use the same row
          workingPortion += dividendStr[dividendIdx];
        }
        ...
        const broughtDownCell: GridCell = {
          ...
          row: currentWorkingPortionRow, // Use the consistent row for this working portion
      ...
      currentRow += 1; // Move to next row for subtraction result
      ...
      currentCells = [
        ...currentCells,
        {
          ...
          value: "−",
          column: VERTICAL_BAR_COLUMN,
          row: currentRow - 1, // Position between product and result
          type: "operation",
          fadeIn: true,
        },
      ];
      ...
      const cellsForSubtractionQuestion = currentCells.map((cell) => {
        ...
        if (
          ...
          cell.row === currentWorkingPortionRow
        ) {
